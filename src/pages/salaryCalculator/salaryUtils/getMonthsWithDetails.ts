...
export const getMonthsWithDetails = ({
	daysWithDetails,
	hourlyPay,
	selectedMonth,
}: Props) => {
	...
	return new Map([
		...(months.map((month) => {
			const monthsDaysWithDetails = [...daysWithDetails.values()].filter(
				(day) => isSameMonth(day.date, month),
			);

			const isMonthInSelectedYear = isSameYear(month, selectedMonth);
			const isMonthSelected = isSameMonth(month, selectedMonth);
			const workdays = monthsDaysWithDetails.filter(
				(day) => day.isWorkday,
			);
			const workhours = monthsDaysWithDetails.reduce(
				(acc, day) => acc + day.workhours,
				0,
			);
